...
class CacheGuessingGameEnv(gym.Env):
  ...
  def __init__(self, env_config={
   ...
   "cache_configs": {
    ...
    self.is_fake_benign = False
    self.is_empty_access = False
    self.benign_reset_victim_add = None
    ...
    # self.num_sets = self.configs['cache_1']['sets']
    self.max_step = window_size
    # print('max step from cache guessing game: ', self.max_step)
  ...
  def print_sample_multiagent_cache(self):
    table = print_cache(self.l1)
    self.vprint(table)

  def print_sample_multiagent(self, obs, reward, done, info, opponent_agent, action_def):
    # observation_bytes = obs['defender'].tobytes()
    # obs_hash = hashlib.sha256(observation_bytes).hexdigest()
    self.vprint('opponent:  ', opponent_agent)
    self.vprint('obs[1]:  ', obs['defender'][0])
    # self.adprint("observations: ", obs_hash)
    # self.adprint("actions: ", observation_bytes)
    # logs = []
    # log_entry = {
    #     "observation_hash": obs_hash,
    #     "action": int(action_def)  
    # }
    
    # # Append logs to the log file
    # name = "hashed.json"
    # log_file = os.path.join(os.getcwd(), name)
  ...
  def step(self, action):
    ...
    if self.is_benign:
      original_action = (action[0] // self.cache_line_size) % len(self.attacker_address_space)
    if self.is_fake_benign:
      original_action = (action[0] // self.cache_line_size) % len(self.attacker_address_space)
    ...
    self.vprint("from cache guessing game: original act: ",original_action, "   act:  ", action, "   addrss  ", address)
    if self.step_count >= self.max_step:#self.window_size - 1:
      r = 2 #
      hit_miss = "NA"
        ...
        if self.allow_victim_multi_access == True or self.victim_accessed == False:
          ...
          if True: #self.configs['cache_1']["rep_policy"] == "plru_pl": no need to distinuish pl and normal rep_policy
            if is_victim_random == True:
                if (self.victim_address_min == 0 and self.victim_address_max == 0):
                  victim_random = random.randint(0, 1)  
                else:
                ...
                # print("is it benign ? ", self.is_benign)
                if self.is_benign or self.is_fake_benign:
                  self.vprint("victim access %d " % self.benign_reset_victim_add)
                  t, evict_addr, [set_index, way_index] = self.l1.read(hex(self.ceaser_mapping(self.benign_reset_victim_add))[2:], self.current_step)#, domain_id='v')
                else:
    ...
    if self.is_fake_benign == False:
      self.state.append([r, victim_accessed, original_action, 0])#self.step_count])
      self.state.popleft()
      self.step_count += 1
    else:
      self.latest_observation = [r, victim_accessed, original_action, 0]
  ...
  def reset(self,
    ...
    if self.cache_state_reset or reset_cache_state or seed != -1:
      ...
      if seed == -1:
        if reset_benign:
          self._randomize_cache(mode='benign', reset_data=reset_data)
        else:
          self._randomize_cache()
      else:
        self.seed_randomization(reset_benign, seed, reset_data) 
  ...
  def seed_randomization(self, reset_benign=False, seed=-1, reset_data=None):   
    if reset_benign:
      return self._randomize_cache(mode="benign", seed= seed, reset_data=reset_data)
    else:  
      return self._randomize_cache(mode="union", seed= seed, reset_data=reset_data)


  def _randomize_cache(self, mode = "union", seed=-1, reset_data=None):
    ...
    for _ in range(self.cache_size * 2):
      if mode == "victim":
        ...
        for i in range(len(reset_data)):
          self.l1.read(hex(reset_data[i])[2:], self.current_step)
          self.current_step += 1
        self.current_step -= (len(reset_data))
        addr = reset_data[-1]
      else:
  ...
  def adprint(self, *args): #action distribution print
    if self.verbose == 2:
      print( " "+" ".join(map(str,args))+" ")
  ...
  def parse_action(self, action):
    # print("--------", len(self.attacker_address_space), action)
    address = 0
    is_guess = 0
    is_victim = 0
    is_flush = 0
    ...
    if self.flush_inst == False:
      if action < len(self.attacker_address_space):
        ...
        if self.is_benign == False and self.is_fake_benign == False:
          is_guess = 1
