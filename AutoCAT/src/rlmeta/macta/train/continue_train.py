...
def main(cfg):
    ...
    if cfg.use_attacker_pool:
        train_model = prepare_model_pool(train_model, cfg)

    defender_params = torch.load(cfg.defender_checkpoint, map_location=cfg.train_device_d)
    train_model_d.load_state_dict(defender_params)
    ...
    if cfg.training_agents == "defender" :
        ...
        if cfg.training_mode == "static" :
            for epoch in range(cfg.num_epochs):  
                a_stats, d_stats = None, None
                a_ctrl.set_phase(Phase.TRAIN, reset=True)
                if epoch % (cfg.attacker_train_epochs + cfg.defender_train_epochs) >= cfg.attacker_train_epochs:
                    # Train Detector
                    print("agent being trained: defender, Epoch: ", epoch)
                    is_attacker = 0
                    agent_d.set_use_history(False)
            ...
            # for epoch in range(10):
                ...
                a_ctrl.set_phase(Phase.TRAIN, reset=True)
                # if turn == 0:
                if epoch < (cfg.attacker_train_epochs + cfg.defender_train_epochs):
                    if epoch % (cfg.attacker_train_epochs + cfg.defender_train_epochs) >= cfg.attacker_train_epochs:
                        # Train Detector
                        print("agent being trained: defender, Epoch: ", epoch)
                        is_attacker = 0
