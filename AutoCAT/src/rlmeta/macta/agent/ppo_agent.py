...
class PPOAgent(PPOAgent):
    ...
    def fake_act(self, obs) -> Action:
        action, logpi, v = self.model.act(
            obs, torch.tensor([self.deterministic_policy]))
        return Action(action, info={"logpi": logpi, "v": v})

    def student_train(self, obs: torch.Tensor, teacher_action):
        student_action = self.model.f_pass(obs, torch.tensor([self.deterministic_policy]))
        ls, gd, isvm_loss, l2_loss, ent_loss = self.model.update_weights(student_action.squeeze(), teacher_action)
        return ls, gd, isvm_loss, l2_loss, ent_loss

    def student_act(self, obs: torch.Tensor):
        # self.model.to(device)
        action = self.model.f_pass(
            obs, torch.tensor([self.deterministic_policy]))
        action = (action >= 0.5)
        action = action.int()
        return action
