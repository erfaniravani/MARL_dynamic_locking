...
class Cache:
    def __init__(self, name, word_size, block_size, n_blocks, associativity,
                 hit_time, write_time, write_back, logger, next_level=None,
                 rep_policy='', prefetcher="none", verbose=False):
        self.name = name
        ...
        elif rep_policy == 'new_plru_pl':
            self.vprint("use new_plru_pl_policy")
            self.rep_policy = new_plru_pl_policy
    ...
    # flush the cache line that contains the address from all cache hierarchy
    ...
    def read(self, address, current_step, pl_opt=-1):
        ...
        if self.prefetcher == "none":    
            ...
            for i in range(len(self.prefetcher_table)):
                ...
                if int(address, 16) + entry["first"] == \
                        entry["second"] * 2 and entry["first"] != -1 and entry["second"] != -1:
                    found = True
                ...
                elif int(address, 16) == entry["first"] + 1 or \
                        int(address, 16) == entry["first"] - 1 and entry["first"] != -1:
    ...
    def read_no_prefetch(self, address, current_step, pl_opt=-1):  # domain_id = 'X'):
        ...
        data_way_index = 0
        r = None
        # Check if this is main memory. Main memory is always a hit
        if not self.next_level:
            ...
            # Parse our address to look through this cache
            block_offset, index, tag = self.parse_address(address)
            # print("block offset, index, tag  ", block_offset, int(index,2), tag)
            # Get the tags in this set
            in_cache = []
            ...
            if tag in in_cache:
                ...
                r, evict_addr, _ = self.next_level.read(address, current_step, pl_opt)
                
                if evict_addr != -1:
                    
                    evict_block_offset, evict_index, evict_tag = \
                        self.parse_address(hex(int(evict_addr, 2))[2:].zfill(8))
                    
                    for i in range(0, len(self.data[evict_index])):
                        if self.data[evict_index][i][0] == evict_tag:
                            self.data[evict_index][i] = \
                                (INVALID_TAG, block.Block(self.block_size, current_step, False, 'x'))
                    ...
                    for slc in self.same_level_caches:
                        evict_block_offset, evict_index, evict_tag = \
                            slc.parse_address(hex(int(evict_addr, 2))[2:].zfill(8))
                        for i in range(0, len(slc.data[evict_index])):
                            if slc.data[evict_index][i][0] == evict_tag:
                                # slc.logger.info
                                slc.data[evict_index][i] = \
                                    (INVALID_TAG, block.Block(slc.block_size, current_step, False, 'x'))
                ...
                if len(in_cache) < self.associativity and self.rep_policy != lru_lock_policy:   
                    # print("can you see this? ", len(self.data[index]), self.data[index], self.set_rep_policy[index].get_locked_array())
                    for i in range(0, len(self.data[index])):
                        if (self.data[index][i][0] == INVALID_TAG):# and (self.set_rep_policy[index].get_locked_array()[i] == 2):
                    ...
                    if self.rep_policy == lru_lock_policy:
                        is_evict, victim_tag, way_index = \
                            self.set_rep_policy[index].find_victim(tag, current_step, self.data[index])
                        
                    else:
                        victim_tag = self.set_rep_policy[index].find_victim(current_step)
                    
                    if victim_tag != INVALID_TAG or \
                            self.rep_policy == lru_lock_policy and is_evict and victim_tag != INVALID_TAG:
                            ...
                            for i in range(0, len(self.data[index])):
                                if self.data[index][i][0] == victim_tag:
                                    if self.data[index][i][1].is_dirty():  
                                        self.logger.info('\tWriting back block ' + address + ' to ' +
                                                         self.next_level.name)
                                        temp, _, _ = self.next_level.write(self.data[index][i][1].address,
                                                                           True, current_step)
                                        r.time += temp.time
                                        break
                        ...
                        evict_addr = victim_tag + index_i + '0' * int(math.log(self.block_size, 2))
        ...
        return r, evict_addr, [data_set_index,data_way_index]
    ...
    def write(self, address, from_cpu, current_step, pl_opt=-1):
        ...
        if not self.next_level:
            ...
            if tag in in_cache:
                ...
                # If there is space in this set, create a new block and
                # set its dirty bit to true if this write is from CPU
    ...
    def get_locked_bits(self): #should return a the locked array --> used for detector's obervation space
        merged_list = []
        for index in self.set_rep_policy:
            l = self.set_rep_policy[index].get_locked_array()
